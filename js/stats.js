...
export function renderStats(container) {
  ...
  container.innerHTML = `
    <div class="border-b border-[#D6CDC0] pb-3 mb-6">
      <h1 class="text-2xl font-display text-[#1B2A4A]">Estadísticas</h1>
    </div>

    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
      <div class="bg-white rounded-xl border border-[#D6CDC0] p-6">
        <h3 class="text-lg font-display text-[#1B2A4A] mb-4">Ranking General</h3>
        ${total === 0 ? `
                  ...
                  <div class="flex-1">
                    <div class="flex justify-between items-center mb-1">
                      <div class="flex items-center gap-2">
                        ...
                        <span class="font-medium text-[#1B2A4A] text-sm">${p.name}</span>
                      </div>
                      <span class="font-bold text-[#1B2A4A] text-sm">${p.count}</span>
                    </div>
        ...
        ` : `
          <div class="flex flex-col items-center">
            <div class="relative w-48 h-48 mb-6">
              ...
              <div class="absolute inset-6 bg-white rounded-full flex items-center justify-center shadow-inner">
                <div class="text-center">
                  <span class="text-3xl font-bold text-[#1B2A4A] block">${total}</span>
                  <span class="text-xs text-[#8B7D6B] uppercase tracking-wider">Total</span>
                </div>
              </div>
            </div>
            <div class="flex flex-wrap gap-5 justify-center">
              ${PEOPLE.map(p => {
                ...
                return `
                  <div class="flex items-center gap-2">
                    <span class="w-3 h-3 rounded-full" style="background-color: ${p.color}"></span>
                    <span class="text-sm text-[#8B7D6B]">${p.name}</span>
                    <span class="text-sm font-semibold text-[#1B2A4A]">${pct}%</span>
                  </div>
                `;
              }).join('')}
            </div>
      ...
      ` : `
        <div class="overflow-x-auto">
          <table class="w-full text-sm">
            <thead>
              <tr class="border-b border-[#D6CDC0]">
                <th class="text-left py-3 px-4 font-medium text-[#8B7D6B]">Mes</th>
                ${PEOPLE.map(p => `<th class="text-center py-3 px-4 font-medium text-[#8B7D6B]">${p.name}</th>`).join('')}
                <th class="text-center py-3 px-4 font-medium text-[#8B7D6B]">Total</th>
              </tr>
            </thead>
            <tbody>
              ${months.map(m => {
                ...
                return `
                  <tr class="border-b border-[#D6CDC0] hover:bg-[#F6F4EF] transition-colors">
                    <td class="py-3 px-4 font-medium text-[#1B2A4A] capitalize">${monthName}</td>
                    ${PEOPLE.map(p => `
                      <td class="text-center py-3 px-4 text-[#8B7D6B]">
                        ${monthly[p.name] > 0 ? `<span class="inline-flex items-center justify-center w-7 h-7 rounded-full text-xs font-semibold" style="background-color: ${p.color}22; color: ${p.color}">${monthly[p.name]}</span>` : '<span class="text-[#D6CDC0]">0</span>'}
                      </td>
                    `).join('')}
                    <td class="text-center py-3 px-4 font-bold text-[#1B2A4A]">${monthTotal}</td>
                  </tr>
                `;
              }).join('')}
            </tbody>
    ...
    <div class="bg-white rounded-xl border border-[#D6CDC0] p-6">
      <h3 class="text-lg font-display text-[#1B2A4A] mb-4">Historial</h3>
      ${records.length === 0 ? `
        <p class="text-[#8B7D6B] text-center py-8">No hay registros aún.</p>
      ` : `
        <div class="overflow-x-auto">
          <table class="w-full text-sm">
            ...
            <tbody>
              ${records.map(([date, record]) => {
                ...
                return `
                  <tr class="border-b border-[#D6CDC0] hover:bg-[#F6F4EF] transition-colors">
                    <td class="py-3 px-4 text-[#8B7D6B]">${formattedDate}</td>
                    <td class="py-3 px-4 text-[#8B7D6B] capitalize">${dayName}</td>
                    <td class="py-3 px-4">
